...
void CuDNNDeconvolutionLayer<Dtype>::Forward_gpu(
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    ...
    {
      // Filters.
      CUDNN_CHECK(cudnnConvolutionBackwardData(handle_[g],
    ...
    int g = 0;
	{
      ...
      if (this->bias_term_ && this->param_propagate_down_[1]) {
        CUDNN_CHECK(cudnnConvolutionBackwardBias(handle_[0*this->group_ + g],
              cudnn::dataType<Dtype>::one,
              bottom_descs_[i],  top_diff + top_offset_ * g,
              cudnn::dataType<Dtype>::one,
      ...
      if (this->param_propagate_down_[0]) {
        ...
        CUDNN_CHECK(cudnnConvolutionBackwardFilter(
              ...
              cudnn::dataType<Dtype>::one,
              bottom_descs_[i],    top_diff + top_offset_ * g,
              top_descs_[i], bottom_data + bottom_offset_ * g,
              conv_descs_[i],
      ...
      if (propagate_down[i]) {
        ...
        CUDNN_CHECK(cudnnConvolutionForward(
              ...
              cudnn::dataType<Dtype>::zero,
              top_descs_[i], bottom_diff + bottom_offset_ * g));
      }
    }
